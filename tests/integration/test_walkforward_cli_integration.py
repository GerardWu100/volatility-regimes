...
def test_walkforward_cli_writes_summary_outputs_and_applies_embargo(
    ...
    def fake_linear_features(
        train_features: pd.DataFrame,
        train_target: pd.Series,
        test_row: pd.Series,
    ) -> float:
        """Record the train-window boundary used for the current test row."""
    ...
    def fake_regime_mean(
        ...
        """Record the train-window boundary used by the regime benchmark."""
        _ = model_type
        _ = min_k
        _ = max_k
        observed_train_windows.append(
    ...
    monkeypatch.setattr(cli, "forecast_linear_features", fake_linear_features)
    monkeypatch.setattr(cli, "forecast_regime_mean", fake_regime_mean)
    ...
    assert len(forecast_panel) == 40
    assert set(metric_summary["n_forecasts"]) == {10}
    ...
    assert observed_train_sizes == {15, 20}
